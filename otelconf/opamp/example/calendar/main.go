...
func realMain() error {
	...
	// Runtime configuration is server-authoritative: after the bootstrap config
	// is applied (and reported) at startup, only the OpAMP server changes the
	// configuration. There is no local runtime reload. Wait for a termination
	// signal and shut down cleanly.
